...
import { metrics, recordRequest, recordCompletion, recordError } from "../helpers/metrics";
...
export const convert = new Elysia().use(userService).post(
  ...
  async ({ body, redirect, jwt, cookie: { auth, jobId }, set }) => {
    ...
    recordRequest(user.id); // 📊 Track request here
    ...
    try {
      ...
      // premium check
      if ((converterName === "tesseract" || converterName === "tableToCSV") &&
          userWithPremium.is_premium !== 1) {
        const userData = db.query("SELECT * FROM users WHERE id = ?").get(user.id);
        return redirect(`/premium-required?email=${encodeURIComponent(userData.email)}&isPremium=false`, 303);
      }
      ...
      recordCompletion(Date.now() - startRequest); // 📊 Track job time
